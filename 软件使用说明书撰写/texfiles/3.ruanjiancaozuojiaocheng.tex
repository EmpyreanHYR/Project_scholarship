...
{\color{myred}
	...
	\end{enumerate}
	导入完成后，系统会弹出校验摘要弹窗，展示成功导入的记录数以及所有警告项（如规范化日期数量、跳过的空行数、清除的无效加分值等），帮助用户快速了解数据质量。
	% 【需补充截图：单文件导入校验摘要弹窗（含警告项），命名为 fig/V3.0_import_validation.png】
	% 【需补充截图：批量导入校验摘要弹窗，命名为 fig/V3.0_batch_validation.png】
}
...
{\color{myred}
	...
	管理员登录后，导航栏“登录”按钮左侧会显示“管理”按钮。点击后打开管理员面板（示意图，待补充实际截图）。面板中以表格列出所有用户的用户名、角色和锁定状态，并提供三个操作按钮：
	\begin{enumerate}
		\item \textbf{解锁账户}：解除被锁定用户的锁定状态。
		\item \textbf{切换角色}：在管理员和评审员之间切换指定用户的角色。
		\item \textbf{删除用户}：从系统中移除指定用户（不可删除当前登录用户）。
	\end{enumerate}
	% 【需补充截图：管理员面板界面，命名为 fig/V3.0_admin_panel.png】
}
...
{\color{myred}
	...
	\end{itemize}
	% 【需补充截图：评审界面中撤销/重做按钮的特写，命名为 fig/V3.0_undo_redo.png】
}

{\color{myred}
	\section{数据统计可视化面板}
	评审界面新增“统计面板”按钮。导入数据并完成评审后，点击该按钮将打开数据可视化窗口（需安装 matplotlib 库：\texttt{pip install matplotlib}）。窗口中以 2x2 网格展示四张统计图表：
	\begin{enumerate}
	...
	\end{enumerate}
	图表窗口底部配有工具栏，支持缩放、平移以及将图表保存为图片文件。

	若系统未安装 matplotlib，点击“统计面板”会弹出提示，引导用户通过 \texttt{pip install matplotlib} 安装。
	% 【需补充截图：统计可视化面板全貌（四张图表），命名为 fig/V3.0_visualization.png】
}

{\color{myred}
	\section{PDF 报告导出}
	版本V3.0在原有Excel导出功能的基础上，新增了PDF格式的评审报告导出（需安装 reportlab 库：\texttt{pip install reportlab}）。评审界面新增了两个PDF导出按钮：

	\subsection{导出PDF报告（个人）}
	点击“导出PDF报告”按钮，将当前学生的评审结果导出为A4格式的PDF文件。报告内容包括：
	\begin{itemize}
	...
	\subsection{批量PDF汇总}
	点击“批量PDF汇总”按钮，将所有学生的评审数据汇总为一份PDF排名表。表格按总分降序排列，包含排名、姓名、学院、班级、学号、各类型得分和总分。第一名以黄色高亮标识。适合打印存档或提交评审委员会审阅。

	若系统未安装 reportlab，点击PDF导出按钮会弹出提示，引导用户通过 \texttt{pip install reportlab} 安装。
	% 【需补充截图：个人PDF报告示例（第一页），命名为 fig/V3.0_pdf_report.png】
	% 【需补充截图：批量PDF汇总报告示例（排名表），命名为 fig/V3.0_pdf_batch.png】
} 