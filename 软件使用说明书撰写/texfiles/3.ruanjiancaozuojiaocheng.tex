...
\textcolor{mygreen}{
	...
	extcolor{myred}{
	\section{（可选）数据库功能启用与历史记录查询（V3.0）}
	版本V3.0新增数据库能力，用于对导入批次、学生信息、申请记录、评审结果与操作日志进行留存与追溯。该功能为可选功能：默认不启用，不影响原有文件导入、评审与导出流程。
	
	\subsection{启用数据库功能}
	（1）在软件根目录复制配置模板文件，生成数据库配置文件：
	\begin{quote}
			exttt{cp db\_config.example.json db\_config.json}
	\end{quote}
	（2）编辑\texttt{db\_config.json}，将\texttt{enabled}设置为\texttt{true}，并填写数据库连接信息（数据库类型、主机、端口、库名、用户名、密码等）。
	（3）初始化数据库表结构（首次启用时执行一次）：
	\begin{quote}
			exttt{python3 -c "from database.migrate import init_database_schema; init_database_schema()"}
	\end{quote}
	（4）验证配置与系统完整性：
	\begin{quote}
