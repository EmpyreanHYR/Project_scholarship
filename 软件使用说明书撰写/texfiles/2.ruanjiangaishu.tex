...
\textcolor{mygreen}{	
	...
	\textcolor{myred}{
	版本V3.0新增了“数据库（可选）”能力：当用户需要对历史评审数据进行查询、导出或追溯操作日志时，可通过配置文件启用数据库功能；当不启用数据库时，软件原有功能不受影响。
}

	\textcolor{myred}{
	其中新增/涉及的关键文件包括：\texttt{db\_config.json}（数据库连接配置，默认不启用，可由\texttt{db\_config.example.json}复制得到）、\texttt{verify\_system.py}（配置与完整性验证脚本），以及\texttt{database/}目录（数据库模块）。
}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{fig/2.0文件导入}
	\caption{文件导入按钮}
	\label{fig:2.0文件导入}
\end{figure}
